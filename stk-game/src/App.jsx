...
export default function App() {
  ...
  const handleCardClick = (clickedCard) => {
    if (
      matchedPairs.includes(clickedCard.pairId) ||
  ...
  const isVictory =
    matchedPairs.length > 0 && matchedPairs.length === cardsData.length / 2;

  // ── Fonction déclenchée par le bouton "Commencer à jouer" de IntroPage ──
    ...
    <div className="app-container">
      ...
      {page === "home" && (
        <div className="stk-hero-section">
          ...
          {/* ── MODIFIÉ : le bouton mène maintenant à "intro" ── */}
          <button
            className="stk-button-hero-large"
      ...
      {/* ════════════════════════════════════════
          PAGE 2 — IntroPage ← NOUVELLE
          ════════════════════════════════════════ */}
      {page === "intro" && (
        <IntroPage
          onStartGame={handleStartGame}   // bouton "Commencer à jouer"
          onBack={() => setPage("home")}  // bouton "← Retour"
        />
      )}

      {/* ════════════════════════════════════════
          PAGE 3 — Jeu (inchangée)
          ════════════════════════════════════════ */}
      {page === "game" && (
        isVictory ? (
          <div className="stk-hero-section">
            <h1 className="stk-hero-title">
              <span className="stk-serif">Félicitations</span><br />
              Écosystème complété.
            </h1>
            <button
              className="stk-button-hero-large"
